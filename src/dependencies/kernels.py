...
def _custom_check_length_scale(length_scale):
    length_scale = np.squeeze(length_scale).astype(float)
    if np.ndim(length_scale) > 1:
        raise ValueError("length_scale cannot be of dimension greater than 1")
    return length_scale
...
class CustomRBF(StationaryKernelMixin, NormalizedKernelMixin, Kernel):
    ...
    A modified version of the RBF kernel with distance SQUARE matrix as input
    Therefore, don't need to compute the distance matrix..
    ...
    def __call__(self, Dxx, Dxz=None, eval_gradient=False):
        """Return the kernel k(X, Y) and optionally its gradient.

        Parameters
        ----------
        Dxx : array, shape (n_samples_X, n_samples_X)
            A pairwise distance SQUARE matrix. D(X, X)

        Dxz : array, shape (n_samples_Y, n_features), (optional, default=None)
            D(X, Z)
        ...
        Returns
        -------
        K : array, shape (Dxx.shape[0], Dxz.shape[0])
            D(X,X) or D(X,Z).T
        ...
        K_gradient : array (opt.), shape (n_samples_X, n_samples_X, n_dims)
            ...
            is True.
        """
        length_scale = _custom_check_length_scale(self.length_scale)
        if Dxz is None:
            dists = Dxx / (length_scale ** 2)
            K = np.exp(-.5 * dists)
            np.fill_diagonal(K, 1)  # diagonal is 1 = exp(0)
        else:
            if eval_gradient:
                raise ValueError(
                    "Gradient can only be evaluated when Dxz is None.")
            dists = Dxz.T / (length_scale ** 2)  # transpose!
            K = np.exp(-.5 * dists)

        if eval_gradient:
            if self.hyperparameter_length_scale.fixed:
                # Hyperparameter l kept fixed
                return K, np.empty((Dxx.shape[0], Dxx.shape[0], 0))
            elif not self.anisotropic or length_scale.shape[0] == 1:
                K_gradient = \
                    (K * dists)[:, :, np.newaxis]
                return K, K_gradient
